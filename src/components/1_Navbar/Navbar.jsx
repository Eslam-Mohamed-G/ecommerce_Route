        ...
        <div className={`navbar w-full backdrop-blur-sm fixed top-0 transition-all ease-in-out duration-500 z-50`}>
            ...
            <div className={`bg-slate-400/30 flex justify-between align-middle ${isScroll ? 'py-3' : 'py-5'}`}>
                <div className='container mx-auto px-4 sm:px-12 flex flex-col'>
                    ...
                    <div className={`w-full transition-all ease-in-out duration-500 overflow-hidden ${isToggle ? user?.token ? 'h-72 pt-5 sm:h-full sm:pt-0' : 'h-48 pt-5 sm:h-full sm:pt-0' : 'h-0 sm:h-full'}`}>
                        <ul className='flex flex-col sm:flex-row gap-5'>
                            ...
                            <li className='flex'><NavLink to={'category'} onClick={() => { handleMenu(); setMenuName("Categories") }} className='w-full border-b border-blue-500 sm:border-none'>Categories</NavLink></li>
