...
function Navbar() {
    ...
    const { products, cart, home } = useParams();
    const handleMenu = () => {
            ...
            <div className={`bg-slate-400/30 flex justify-between align-middle ${isScroll ? 'py-3' : 'py-5'}`}>
                <div className='container mx-auto px-4 sm:px-12 flex flex-col'>
                    <div className='flex justify-between sm:hidden'>
                        <h1 className='capitalize'>{products} {cart} {home}</h1>
                    ...
                    <div className={`w-full transition-all ease-in-out duration-500 overflow-hidden ${isToggle ? 'h-56 pt-5 sm:h-full sm:pt-0' : 'h-0 sm:h-full'}`}>
                        <ul className='flex flex-col sm:flex-row gap-5'>
                            <li><NavLink to={'/'} onClick={() => { handleMenu(); navigate('/home'); }}>Home</NavLink></li>
                            <li><NavLink to={'/products'} onClick={() => { handleMenu(); navigate('/Products'); }}>Products</NavLink></li>
                            <li><NavLink onClick={() => { handleMenu(); navigate("/Brands") }}>Brands</NavLink></li>
                            <li><NavLink onClick={() => { handleMenu(); navigate("/Categories") }}>Categories</NavLink></li>
